...
(local init (require :fen_web.demo))
(local manifest (require :fen_web.demo.manifest))
    ...
    (describe "manifest"
      (fn []
        (it "excludes the state module from reload and lists behavior modules"
          (fn []
            (assert.are.equal :fen_web.demo manifest.entry-module)
            (assert.are.equal :dom manifest.presenter)
            (let [excluded {}]
              (each [_ m (ipairs manifest.reload-exclude)] (tset excluded m true))
              (assert.is_true (. excluded :fen_web.demo.state)))
            (let [reloaded {}]
              (each [_ m (ipairs manifest.reload-modules)] (tset reloaded m true))
              (assert.is_nil (. reloaded :fen_web.demo.state))
              (assert.is_true (. reloaded :fen_web.demo))))))))
